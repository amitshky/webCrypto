...
(async () =>
{
	...
	//const rsaPrivateKey = await crypto.subtle.exportKey('pkcs8', rsaKey.privateKey!);
	//const rsaPublicKey  = await crypto.subtle.exportKey('spki', rsaKey.publicKey!);

	// sign plaintext
	const signature = await crypto.subtle.sign({ name: 'RSASSA-PKCS1-v1_5' }, rsaKey.privateKey!, plaintext);
